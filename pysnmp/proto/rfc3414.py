"""
   ...
   Written by Ilya Etingof <ilya@glas.net>, 2003.
"""
# Module public names
#__all__ = [ ]

from pysnmp.proto import rfc1157, rfc1902, rfc1905, rfc3411, error
from pysnmp.smi import rfc3414
from pysnmp.asn1.error import Asn1Error

# Local Configuration Datastore (lcd)

def lcdGet(mib, idx):
    usmUserEngineID = mib.searchNode('.1.3.6.1.6.3.15.1.2.2.1.1.%d' % idx)
    usmUserName = mib.searchNode('.1.3.6.1.6.3.15.1.2.2.1.2.%d' % idx)
    usmUserAuthProtocol = mib.searchNode('.1.3.6.1.6.3.15.1.2.2.1.5.%d' % idx)
    usmUserPrivProtocol = mib.searchNode('.1.3.6.1.6.3.15.1.2.2.1.8.%d' % idx)
    return (usmUserEngineID['type'].getTerminal(),
            usmUserName['type'].getTerminal(),
            usmUserAuthProtocol['type'].getTerminal(),
            usmUserPrivProtocol['type'].getTerminal())

def lcdSet(mib, idx, **kwargs):
    usmUserEngineID = kwargs.get('usmUserEngineID', None)
    usmUserName = kwargs.get('usmUserName', None)
    usmUserAuthProtocol = kwargs.get('usmUserAuthProtocol', None)
    usmUserPrivProtocol = kwargs.get('usmUserPrivProtocol', None)
...
if __name__ == '__main__':
    ...
    mib = oidtree.Root()
    mib.attachNode(rfc1907.Snmp())
    mib.attachNode(rfc3411.SnmpFrameworkMib())
    mib.attachNode(rfc3412.SnmpMpdMib())
    
    m = UserBasedSecurityModule(mib)
